...
describe "PG Delete" do
  it "deletes with simple where clause" do
    in_transaction do |conn|
      Employees.select(Employees.id).where(Employees.id.eq(1)).query_one?(conn, Int32).should_not be_nil

      Employees.delete.where(Employees.id.eq(1)).exec(conn)

      Employees.select(Employees.id).where(Employees.id.eq(1)).query_one?(conn, Int32).should be_nil
    end
  end
